...
$result = send_request(array_merge($ses_info, ['action' => 'groups_list', 'type' => 'product', 'status' => $status]), 'noms');
...
function fncRenderGroupsTree($nodes, $level = 0) {
    foreach ($nodes as $node) {
        ...
        ?>
        <div class="tree-node">
            <div class="tree-row itemTr<?php echo $node['is_active'] ? '' : ' tree-row-archived'; ?>" data-id="<?php echo (int)$node['id']; ?>">
                <?php if ($has_children): ?>
