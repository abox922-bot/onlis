...
$result = send_request(array_merge($ses_info, [
    'action' => 'organization_position_info',
    'id'     => $id,
]), 'orgs');
...
<form id="formInfo">
    <div class="row">
        ...
        <div class="col-12 mb-3">
            <label for="inpPosName" class="my-input-label">Название</label>
            <input type="text"
        ...
        <?php if ($result['staff_count'] ?? 0): ?>
        <div class="col-12 mb-2">
            <small class="text-muted">
                Сотрудников: <?php echo (int)$result['staff_count']; ?>
            </small>
        </div>
        <?php endif; ?>
        ...
        <div class="col-12 mb-3">
            <div class="form-check form-switch">
                <input class="form-check-input form-inp" type="checkbox" role="switch"
                    id="chckPosActive"
                    data-name="pos-active"
                    data-type="check"
                    <?php echo !empty($result['is_active']) ? 'checked' : ''; ?>
                    <?php echo $has_staff ? 'disabled' : ''; ?>>
                <label class="form-check-label" for="chckPosActive" id="lblPosActive">
                    <?php echo !empty($result['is_active']) ? 'Активная' : 'Архивная'; ?>
                </label>
            </div>
            <?php if ($has_staff): ?>
