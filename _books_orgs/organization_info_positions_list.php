...
$result = send_request(array_merge($ses_info, [
    'action' => 'organization_positions_list',
    'id'     => $id,
]), 'orgs');
...
<?php if (empty($result)): ?>
    <div class="empty-hint">
        <i class="bi bi-person-badge empty-hint__icon"></i>
        <div class="empty-hint__text">Должностей не найдено</div>
    </div>
<?php else: ?>
    <table class="table table-sm table-hover mt-2">
        <tbody>
            <?php foreach ($result as $value): ?>
                <tr class="posTr <?php echo !$value['is_active'] ? 'table-secondary' : ''; ?>"
