    ...
    $("#mainModalBody").load(path.href, function(){
      ...
      $("#formNew").submit(function(e){
        ...
        if (crt_arr["all_good"] && confirm("Сохранить?")) {
          ...
          $.ajax({type: "POST",	url: rqst_path.href, data: {params: JSON.stringify(crt_arr["params"]), action: "new_country", module: "geo", return_data: 1},	success: function(data){
...
function infoLoadFunction(item_id) {
  ...
  $("#mainModalBody").load(path.href, {id: item_id}, function(){
    ...
    $("#formInfo").submit(function(e){
      ...
      if (crt_arr["all_good"] && confirm("Сохранить?")) {
        $("#btnSave, #divSaveLoading").toggleClass("d-none");
        $.ajax({type: "POST",	url: rqst_path.href, data: {params: JSON.stringify(crt_arr["params"]), action: "upd_country", module: "geo", return_data: 0},	success: function(){
          listLoadFunction();
          main_modal.hide();
        }});
      }
